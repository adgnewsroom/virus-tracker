function d3waffle() {
  ...
  function chart(selection) {
    ...
    selection.each(function(data) {
      ...
      var total = d3v3.sum(data, function(d) { return d.value; });
      ...
      var totalscales = d3v3.sum(data, function(d){ return d.scalevalue; })
      var cols = Math.ceil(totalscales/rows);
      var griddata = cartesianprod(d3v3.range(cols), d3v3.range(rows));
      var detaildata = [];

      data.forEach(function(d){
        d3v3.range(d.scalevalue).forEach(function(e){
      ...
      legend.append('text')
            ...
            .attr("transform", function(d) { return "translate(" + gridSize*2 + "," + 0.4*gridSize  + ")"; })
      ...
      function mouseover(d){
        ...
        txt = "<b>" +el.name + "</b><br>" + d3v3.format(',')(el.value) + "<br>(" + d3v3.format(".0%")(el.percent) + ")"
        tooltip.html(txt);

        d3v3.select("#" + idcontainer).selectAll("text").style("opacity", 1);
        d3v3.select("#" + idcontainer).selectAll("text." + d.class).style("opacity", 1);
      }
      ...
      function mousemove(d){
        tooltip
            .style("left", (d3v3.event.pageX + 0 ) + "px")
            .style("top", (d3v3.event.pageY + - 70) + "px");
      }
